...
class EvaluationInspectionCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, **kwargs):  # noqa: D401 - HF callback signature
        ...
        # Log qualitative samples to TensorBoard if requested. Skip directory writes here since
        # ``save_inspection_artifacts`` already handles filesystem persistence.
...
def collect_samples_for_inspection(
    ...
    *,
    ...
    max_samples = min(args.num_samples_for_report, total_records)
...
def main():
    ...
    if inspection_dataset is not None and args.max_eval_samples:
        inspection_dataset = inspection_dataset.select(
            range(min(args.max_eval_samples, len(inspection_dataset)))
        )
